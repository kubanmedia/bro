...
describe.skip('IdeClient', () => {
  it('reads port from file and connects', async () => {
    ...
    const portFile = path.join(os.tmpdir(), `anus-ide-server-${pid}.json`);
    fs.writeFileSync(portFile, JSON.stringify({ port }));
    process.env['ANUS_CLI_IDE_WORKSPACE_PATH'] = process.cwd();
    ...
    await server.stop();
    delete process.env['ANUS_CLI_IDE_WORKSPACE_PATH'];
    // Reset instance
...
describe('IdeClient fallback connection logic', () => {
  ...
  beforeEach(async () => {
    pid = process.pid;
    portFile = path.join(os.tmpdir(), `anus-ide-server-${pid}.json`);
    server = new TestMcpServer();
    envPort = await server.start();
    process.env['ANUS_CLI_IDE_SERVER_PORT'] = String(envPort);
    process.env['TERM_PROGRAM'] = 'vscode';
    process.env['ANUS_CLI_IDE_WORKSPACE_PATH'] = process.cwd();
    // Reset instance
    IdeClient.instance = undefined;
  ...
  afterEach(async () => {
    await server.stop();
    delete process.env['ANUS_CLI_IDE_SERVER_PORT'];
    delete process.env['ANUS_CLI_IDE_WORKSPACE_PATH'];
