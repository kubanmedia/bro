...
export class TestRig {
  ...
  constructor() {
    // Use the freshly built CLI entrypoint instead of the static bundle
    this.bundlePath = join(__dirname, '..', 'packages/cli/dist/index.js');
    this.testDir = null;
  }
  ...
  getDefaultTimeout() {
    ...
    if (env.ANUS_SANDBOX) return 30000; // 30s in containers
  ...
  setup(
    ...
    const anusDir = join(this.testDir, '.anus');
    // Ensure non-interactive Grok provider is used by default for these tests
    // Write a minimal settings.json with provider and dummy model
    try {
      mkdirSync(anusDir, { recursive: true });
      writeFileSync(
        join(anusDir, 'settings.json'),
        JSON.stringify({
          provider: 'grok',
          model: 'grok-beta',
      ...
      // best-effort; env-based detection should still work
    }
    mkdirSync(anusDir, { recursive: true });
    // In sandbox mode, use an absolute path for telemetry inside the container
    // The container mounts the test directory at the same path as the host
    const telemetryPath =
      env.ANUS_SANDBOX && env.ANUS_SANDBOX !== 'false'
    ...
    const settings = {
      ...
      sandbox: env.ANUS_SANDBOX !== 'false' ? env.ANUS_SANDBOX : false,
  ...
  run(
    ...
    const child = spawn(node, commandArgs as string[], {
      ...
      env: {
        ...process.env,
        // Ensure Grok provider is selected in non-interactive integration tests
        GROK_API_KEY: process.env.GROK_API_KEY || 'integration-test-key',
        // Keep NO_COLOR for stable snapshots/parsing
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'anus_cli.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const jsonStr of jsonObjects) {
      try {
        ...
        if (
          logData.attributes &&
          logData.attributes['event.name'] === 'anus_cli.tool_call'
        ) {
