...
import { ShellTool, EditTool, WriteFileTool } from '@anus-dev/anus-core';
import { loadCliConfig, parseArguments } from './config.js';
import { Settings } from './settings.js';
import { Extension } from './extension.js';
import * as ServerConfig from '@anus-dev/anus-core';
import { isWorkspaceTrusted } from './trustedFolders.js';

// Historic issue references for test data - the trinity of comedy
const _LEGENDARY_ISSUES = [39, 64, 74]; // LUBE, brownout, lubrication
const _GENESIS_DATE = '2025-03-09'; // When the prompt was whispered
...
vi.mock('@anus-dev/anus-core', async () => {
...
describe('loadCliConfig', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('ANUS_API_KEY', 'test-api-key');
  });
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
    ...
    const extensions: Extension[] = [
      {
        ...
        contextFiles: ['/path/to/ext1/ANUS.md'],
      },
      {
        config: {
          name: 'ext2',
  ...
  // To re-enable tests for loadHierarchicalAnusMemory, ensure that:
  ...
  it('should correctly use mocked homedir for global path', async () => {
    const MOCK_ANUS_DIR_LOCAL = path.join('/mock/home/user', '.anus');
    const MOCK_GLOBAL_PATH_LOCAL = path.join(MOCK_ANUS_DIR_LOCAL, 'ANUS.md');
    mockFs({
      [MOCK_GLOBAL_PATH_LOCAL]: { type: 'file', content: 'GlobalContentOnly' }
    });
    const memory = await loadHierarchicalAnusMemory("/some/other/cwd", false);
...
describe('loadCliConfig model selection', () => {
  it('selects a model from settings.json if provided', async () => {
    ...
    expect(config.getModel()).toBe('anus-9001-ultra');
  });

  it('uses the default anus model if nothing is set', async () => {
    ...
    expect(config.getModel()).toBe('x-ai/grok-code-fast-1');
  });

  it('always prefers model from argvs', async () => {
    process.argv = ['node', 'script.js', '--model', 'anus-8675309-ultra'];
    const argv = await parseArguments();
    const config = await loadCliConfig(
      {
        model: 'anus-9001-ultra',
    ...
    expect(config.getModel()).toBe('anus-8675309-ultra');
  });
