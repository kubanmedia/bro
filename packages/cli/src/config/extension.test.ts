...
describe('loadExtensions', () => {
  ...
  beforeEach(() => {
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'anus-cli-test-workspace-'),
    );
    tempHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'anus-cli-test-home-'));
  ...
  it('should load context file path when ANUS.md is present', () => {
    const workspaceExtensionsDir = path.join(
      tempWorkspaceDir,
      EXTENSIONS_DIRECTORY_NAME,
    );
    ...
    expect(ext1?.contextFiles).toEqual([
      path.join(workspaceExtensionsDir, 'ext1', 'ANUS.md'),
    ]);
...
function createExtension(
  ...
  if (addContextFile) {
    fs.writeFileSync(path.join(extDir, 'ANUS.md'), 'context');
  }
