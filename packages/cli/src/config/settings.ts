...
export const SETTINGS_DIRECTORY_NAME = '.anus';
export const USER_SETTINGS_DIR = path.join(homedir(), SETTINGS_DIRECTORY_NAME);
export const USER_SETTINGS_PATH = path.join(USER_SETTINGS_DIR, 'settings.json');
export const DEFAULT_EXCLUDED_ENV_VARS = ['DEBUG', 'DEBUG_MODE'];

export function getSystemSettingsPath(): string {
  if (process.env['ANUS_CLI_SYSTEM_SETTINGS_PATH']) {
    return process.env['ANUS_CLI_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/AnusCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\anus-cli\\settings.json';
  } else {
    return '/etc/anus-cli/settings.json';
  }
}
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    // Check for .env under ANUS_DIR only
    const anusEnvPath = path.join(currentDir, ANUS_DIR, '.env');
    if (fs.existsSync(anusEnvPath)) {
      return anusEnvPath;
    }
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeAnusEnvPath = path.join(homedir(), ANUS_DIR, '.env');
      if (fs.existsSync(homeAnusEnvPath)) {
        return homeAnusEnvPath;
      }
...
export function loadEnvironment(settings?: Settings): void {
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(ANUS_DIR);
