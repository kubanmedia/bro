...
import { useAnusStream } from './hooks/useAnusStream.js';
...
interface MockServerConfig {
  ...
  getAnusMdFileCount: Mock<() => number>;
  setAnusMdFileCount: Mock<(count: number) => void>;
  ...
  getAllAnusMdFilenames: Mock<() => string[]>;
  getAnusClient: Mock<() => AnusClient | undefined>;
  getUserTier: Mock<() => Promise<string | undefined>>;
  getIdeClient: Mock<() => { getCurrentIde: Mock<() => string | undefined> }>;
}

// Mock @anus-dev/anus-core and its Config class
vi.mock('@anus-dev/anus-core', async (importOriginal) => {
  const actualCore =
    await importOriginal<typeof import('@anus-dev/anus-core')>();
  const ConfigClassMock = vi
    ...
    .mockImplementation((optionsPassedToConstructor) => {
      ...
      return {
        ...
        anusMdFileCount: opts.anusMdFileCount || 0,
        ...
        setUserMemory: vi.fn(),
        getAnusMdFileCount: vi.fn(() => opts.anusMdFileCount || 0),
        setAnusMdFileCount: vi.fn(),
        ...
        getAllAnusMdFilenames: vi.fn(() => ['ANUS.md']),
...
const { isGitRepository: mockedIsGitRepository } = vi.mocked(
  await import('@anus-dev/anus-core'),
);
...
describe('App UI', () => {
  ...
  const createMockSettings = (
    ...
    const workspaceSettingsFile: SettingsFile = {
      path: '/workspace/.anus/settings.json',
  ...
  describe('handleAutoUpdate', () => {
    ...
    afterEach(() => {
      delete process.env.ANUS_CLI_DISABLE_AUTOUPDATER;
    });
    ...
    it('should not auto-update if ANUS_CLI_DISABLE_AUTOUPDATER is true', async () => {
      mockedIsGitRepository.mockResolvedValue(false);
      process.env.ANUS_CLI_DISABLE_AUTOUPDATER = 'true';
      const info: UpdateObject = {
        update: {
          name: '@anus-dev/code',
          latest: '1.1.0',
  ...
  it('should display active file and other context', async () => {
    ...
    mockConfig.getAnusMdFileCount.mockReturnValue(1);
    mockConfig.getAllAnusMdFilenames.mockReturnValue(['ANUS.md']);
    ...
    await Promise.resolve();
    expect(lastFrame()).toContain(
      'Using: 1 open file (ctrl+g to view) | 1 ANUS.md file',
    );
  });

  it('should display default "ANUS.md" in footer when contextFileName is not set and count is 1', async () => {
    mockConfig.getAnusMdFileCount.mockReturnValue(1);
    mockConfig.getAllAnusMdFilenames.mockReturnValue(['ANUS.md']);
    ...
    expect(lastFrame()).toContain('Using: 1 ANUS.md file');
  });

  it('should display default "ANUS.md" with plural when contextFileName is not set and count is > 1', async () => {
    mockConfig.getAnusMdFileCount.mockReturnValue(2);
    mockConfig.getAllAnusMdFilenames.mockReturnValue(['ANUS.md', 'ANUS.md']);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
    ...
    currentUnmount = unmount;
    await Promise.resolve();
    expect(lastFrame()).toContain('Using: 2 ANUS.md files');
  });
  ...
  it('should display custom contextFileName in footer when set and count is 1', async () => {
    ...
    mockConfig.getAnusMdFileCount.mockReturnValue(1);
    mockConfig.getAllAnusMdFilenames.mockReturnValue(['AGENTS.md']);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
  ...
  it('should display a generic message when multiple context files with different names are provided', async () => {
    ...
    mockConfig.getAnusMdFileCount.mockReturnValue(2);
    mockConfig.getAllAnusMdFilenames.mockReturnValue([
      'AGENTS.md',
      'CONTEXT.md',
    ]);
    mockConfig.getDebugMode.mockReturnValue(false);
  ...
  it('should display custom contextFileName with plural when set and count is > 1', async () => {
    ...
    mockConfig.getAnusMdFileCount.mockReturnValue(3);
    mockConfig.getAllAnusMdFilenames.mockReturnValue([
      'MY_NOTES.TXT',
      'MY_NOTES.TXT',
      'MY_NOTES.TXT',
  ...
  it('should not display context file message if count is 0, even if contextFileName is set', async () => {
    ...
    mockConfig.getAnusMdFileCount.mockReturnValue(0);
    mockConfig.getAllAnusMdFilenames.mockReturnValue([]);
    mockConfig.getDebugMode.mockReturnValue(false);
    mockConfig.getShowMemoryUsage.mockReturnValue(false);
  ...
  it('should display ANUS.md and MCP server count when both are present', async () => {
    mockConfig.getAnusMdFileCount.mockReturnValue(2);
    mockConfig.getAllAnusMdFilenames.mockReturnValue(['ANUS.md', 'ANUS.md']);
  ...
  it('should display only MCP server count when ANUS.md count is 0', async () => {
    mockConfig.getAnusMdFileCount.mockReturnValue(0);
    mockConfig.getAllAnusMdFilenames.mockReturnValue([]);
    mockConfig.getMcpServers.mockReturnValue({
      server1: {} as MCPServerConfig,
  ...
  describe('when no theme is set', () => {
    ...
    it('should display theme dialog if NO_COLOR is not set', async () => {
      ...
      expect(lastFrame()).toContain(
        'Getting to the bottom of this... (esc to cancel',
      );
    });
  ...
  it('should render correctly with the prompt input box', () => {
    vi.mocked(useAnusStream).mockReturnValue({
  ...
  describe('with initial prompt from --prompt-interactive', () => {
    it('should submit the initial prompt automatically', async () => {
      ...
      mockConfig.getAnusClient.mockReturnValue({
        isInitialized: vi.fn(() => true),
        getUserTier: vi.fn(),
      } as unknown as AnusClient);
  ...
  describe('auth validation', () => {
    it('should call validateAuthMethod when useExternalAuth is false', async () => {
      ...
      mockSettings = createMockSettings({
        workspace: {
          selectedAuthType: 'USE_ANUS' as AuthType,
