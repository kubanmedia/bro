...
import { decodeTagName } from '@anus-dev/anus-core';
...
const getSavedChatTags = async (
  ...
  const anusDir = context.services.config?.getProjectTempDir();
  if (!anusDir) {
    return [];
  }
  try {
    ...
    const files = await fsPromises.readdir(anusDir);
    const chatDetails: Array<{ name: string; mtime: Date }> = [];

    for (const file of files) {
      if (file.startsWith(file_head) && file.endsWith(file_tail)) {
        const filePath = path.join(anusDir, file);
...
const saveCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<SlashCommandActionReturn | void> => {
    ...
    const chat = await config?.getAnusClient()?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
  ...
  action: async (context, args) => {
    ...
    for (const item of conversation) {
      ...
      if (i > 2 || !hasSystemPrompt) {
        uiHistory.push({
          type: (item.role && rolemap[item.role]) || MessageType.ANUS,
          text,
