...
import { loadServerHierarchicalMemory } from '@anus-dev/anus-core';
      ...
      action: async (context: CommandContext, args: string) => {
        ...
        try {
          if (config.shouldLoadMemoryFromIncludeDirectories()) {
            ...
            context.ui.setAnusMdFileCount(fileCount);
          }
          addItem(
            {
              type: MessageType.INFO,
              text: `Successfully added ANUS.md files from the following directories if there are:\n- ${added.join('\n- ')}`,
            },
            Date.now(),
          );
        } catch (error) {
        ...
        if (added.length > 0) {
          const anus = config.getAnusClient();
          if (anus) {
            await anus.addDirectoryContext();
          }
          addItem(
            {
              type: MessageType.INFO,
