...
import {
  Config,
  DetectedIde,
  ANUS_CLI_COMPANION_EXTENSION_NAME,
  IDEConnectionStatus,
  getIdeInfo,
  getIdeInstaller,
  IdeClient,
    ...
    return {
      ...
      action: (): SlashCommandActionReturn =>
        ({
          type: 'message',
          messageType: 'error',
          content: `IDE integration is not supported in your current environment. To use this feature, run ANUS in one of these supported IDEs: ${Object.values(
            DetectedIde,
          )
    ...
    action: async (context) => {
      ...
      if (!installer) {
        context.ui.addItem(
          {
            type: 'error',
            text: `No installer is available for ${ideClient.getDetectedIdeDisplayName()}. Please install the '${ANUS_CLI_COMPANION_EXTENSION_NAME}' extension manually from the marketplace.`,
          },
          Date.now(),
        );
        return;
