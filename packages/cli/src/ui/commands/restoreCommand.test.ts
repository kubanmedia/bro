...
import { Config, GitService } from '@anus-dev/anus-core';
...
describe('restoreCommand', () => {
  ...
  beforeEach(async () => {
    ...
    anusTempDir = path.join(testRootDir, '.anus');
    checkpointsDir = path.join(anusTempDir, 'checkpoints');
    ...
    mockConfig = {
      ...
      getProjectTempDir: vi.fn().mockReturnValue(anusTempDir),
      getAnusClient: vi.fn().mockReturnValue({
  ...
  describe('action', () => {
    it('should return an error if temp dir is not found', async () => {
      ...
      expect(
        ...
        type: 'message',
        messageType: 'error',
        content: 'Could not determine the .anus directory path.',
      });
    });
