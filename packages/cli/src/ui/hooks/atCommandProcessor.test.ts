...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectAnusIgnore: () => true,
  ...
  describe('anus-ignore filtering', () => {
    it('should skip anus-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.anusignore'),
        'build/output.js',
      );
      const anusIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${anusIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${anusIgnoredFile} is anus-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nAnus-ignored: ${anusIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .anusignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.anusignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed anus-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.anusignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${anusIgnoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${validFile} @${anusIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${validFile}:\n` },
