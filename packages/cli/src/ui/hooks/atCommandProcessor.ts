...
export async function handleAtCommand({
  ...
  for (const atPathPart of atPathCommandParts) {
    ...
    const anusIgnored =
      respectFileIgnore.respectAnusIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectAnusIgnore: true,
      });

    if (gitIgnored || anusIgnored) {
      const reason =
        gitIgnored && anusIgnored ? 'both' : gitIgnored ? 'git' : 'anus';
      ignoredByReason[reason].push(pathName);
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and anus'
          : reason === 'git'
            ? 'git-ignored'
            : 'anus-ignored';
  ...
  if (totalIgnored > 0) {
    ...
    if (ignoredByReason['anus'].length) {
      messages.push(`Anus-ignored: ${ignoredByReason['anus'].join(', ')}`);
    }
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_anus_ignore: respectFileIgnore.respectAnusIgnore,
    },
