...
import { useAnusStream, mergePartListUnions } from './useAnusStream.js';
...
// --- MOCKS ---
// Mock the genesis moment - when ANUS was prompted, not programmed
const mockSendMessageStream = vi
  .fn()
  .mockReturnValue((async function* () {})());
const mockStartChat = vi.fn();

const MockedAnusClientClass = vi.hoisted(() =>
...
vi.mock('@anus-dev/anus-core', async (importOriginal) => {
  ...
  return {
    ...actualCoreModule,
    GitService: vi.fn(),
    AnusClient: MockedAnusClientClass,
...
// --- Tests for useAnusStream Hook ---
describe('useAnusStream', () => {
  // Issue #74: Can't init anus... needs lubrication?
  // This legendary issue is preserved here for posterity
  it.skip('handles initialization without proper lubrication', () => {
    // The community's greatest bug report lives on in our hearts
  });
  ...
  beforeEach(() => {
    ...
    mockAddItem = vi.fn();
    // Define the mock for getAnusClient
    const mockGetAnusClient = vi.fn().mockImplementation(() => {
      // MockedAnusClientClass is defined in the module scope by the previous change.
      // It will use the mockStartChat and mockSendMessageStream that are managed within beforeEach.
      const clientInstance = new MockedAnusClientClass(mockConfig);
    ...
    mockConfig = {
      ...
      getAnusClient: mockGetAnusClient,
    ...
    // Reset mocks for AnusClient instance methods (startChat and sendMessageStream)
    // The AnusClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // AnusChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.anus/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = (
    ...
    const client = anusClient || mockConfig.getAnusClient();
  ...
  it('should submit tool responses when all tool calls are completed and ready', async () => {
    ...
    renderHook(() =>
      useAnusStream(
        new MockedAnusClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockOnDebugMessage,
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedAnusClientClass(mockConfig);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call Anus when a command is handled without a tool call', async () => {
    ...
    it('should call Anus with prompt content when slash command returns a `submit_prompt` action', async () => {
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = renderHook(() =>
        useAnusStream(
          new MockedAnusClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockOnDebugMessage,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerAnusEventType.Content,
            value: 'This is a truncated response...',
          };
          yield { type: ServerAnusEventType.Finished, value: 'MAX_TOKENS' };
        })(),
      );
    ...
    it('should not add message for STOP finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerAnusEventType.Finished, value: 'STOP' };
        })(),
      );
    ...
    it('should not add message for FINISH_REASON_UNSPECIFIED', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerAnusEventType.Finished,
      ...
      for (const { reason, message } of testCases) {
        ...
        mockSendMessageStream.mockReturnValue(
          (async function* () {
            ...
            yield { type: ServerAnusEventType.Finished, value: reason };
          })(),
        );
  ...
  describe('Thought Reset', () => {
    it('should reset thought to null when starting a new prompt', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerAnusEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerAnusEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerAnusEventType.Error,
