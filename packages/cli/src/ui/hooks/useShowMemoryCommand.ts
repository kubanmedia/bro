...
export function createShowMemoryAction(
  ...
  return async () => {
    ...
    const fileCount = config.getAnusMdFileCount();
    ...
    if (currentMemory && currentMemory.trim().length > 0) {
      ...
      addMessage({
        ...
        content:
          fileCount > 0
            ? 'Hierarchical memory (ANUS.md or other context files) is loaded but content is empty.'
            : 'No hierarchical memory (ANUS.md or other context files) is currently loaded.',
        timestamp: new Date(),
      });
    }
  };
