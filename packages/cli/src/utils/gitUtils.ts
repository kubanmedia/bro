...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-anus-cli/releases/latest`;
    ...
    console.debug(`Failed to determine latest run-anus-cli release:`, _error);
    throw new Error(
      `Unable to determine the latest run-anus-cli release on GitHub.`,
    );
  }
};
