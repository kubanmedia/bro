...
export async function getInstallationInfo(
  ...
  try {
    ...
    // Check for Homebrew - try both old and new package names
    if (process.platform === 'darwin') {
      try {
        // Check for anus package first
        childProcess.execSync('brew list -1 | grep -q "^anus$"', {
          stdio: 'ignore',
        });
        return {
        ...
        // Try legacy anus-cli name
        try {
          childProcess.execSync('brew list -1 | grep -q "^anus-cli$"', {
            stdio: 'ignore',
          });
          return {
            packageManager: PackageManager.HOMEBREW,
          ...
          // Neither package installed via brew.
    ...
    const packageName = packageJson?.name || 'anus';
