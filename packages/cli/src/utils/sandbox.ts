...
import { Config, SandboxConfig } from '@anus-dev/anus-core';
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'anus-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'anus-cli-sandbox';
const SANDBOX_PROXY_NAME = 'anus-cli-sandbox-proxy';
...
function entrypoint(workdir: string): string[] {
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : process.env['DEBUG']
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which anus)`
        : 'anus';
...
export async function start_sandbox(
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      // start and set up proxy if ANUS_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env['ANUS_SANDBOX_PROXY_COMMAND'];
    ...
    // determine full path for anus-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under anus-cli repo
    //
    // note this can only be done with binary linked from anus-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('anus-cli/packages/')) {
        console.error(
          'ERROR: cannot build sandbox using installed anus binary; ' +
            'run `npm link ./packages/cli` under anus-cli repo to switch to linked binary.',
        );
        process.exit(1);
      } else {
        ...
        execSync(
          `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
          {
            ...
            env: {
              ...process.env,
              ANUS_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
            },
          },
        );
      }
    ...
    if (!(await ensureSandboxImageIsPresent(config.command, image))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the anus-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify anus-cli-dev@google.com if the issue persists.';
      console.error(
    ...
    // ANUS_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['ANUS_SANDBOX_PROXY_COMMAND'];
    ...
    // copy ANUS_API_KEY(s)
    if (process.env['ANUS_API_KEY']) {
      args.push('--env', `ANUS_API_KEY=${process.env['ANUS_API_KEY']}`);
    }
    ...
    // copy ANUS_MODEL
    if (process.env['ANUS_MODEL']) {
      args.push('--env', `ANUS_MODEL=${process.env['ANUS_MODEL']}`);
    }
    ...
    for (const envVar of [
      'ANUS_CLI_IDE_SERVER_PORT',
      'ANUS_CLI_IDE_WORKSPACE_PATH',
      'TERM_PROGRAM',
    ]) {
    ...
    if (process.env['ANUS_CLI_INTEGRATION_TEST'] === 'true') {
      args.push('--user', 'root');
      userFlag = '--user root';
      ...
      // finally switch to that user to run the anus process. This is
