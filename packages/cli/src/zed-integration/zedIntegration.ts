...
export async function runZedIntegration(
  ...
  new acp.AgentSideConnection(
    (client: acp.Client) =>
      new AnusAgent(config, settings, extensions, argv, client),
    stdout,
    stdin,
  );
}
...
class AnusAgent {
  ...
  async initialize(
    ...
    const authMethods = [
      ...
      {
        id: AuthType.USE_GROK,
        name: 'Use Grok API key',
        description: 'Requires setting the `GROK_API_KEY` environment variable',
      },
      {
  ...
  async newSession({
    ...
    const chat = await anusClient.startChat();
