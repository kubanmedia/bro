...
import { setAnusMdFilename as mockSetAnusMdFilename } from '../tools/memoryTool.js';
import {
...
vi.mock('../tools/memoryTool', () => ({
  MemoryTool: vi.fn(),
  setAnusMdFilename: vi.fn(),
  getCurrentAnusMdFilename: vi.fn(() => 'ANUS.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'ANUS.md',
  ANUS_CONFIG_DIR: '.anus',
}));
...
vi.mock('../core/client.js', () => ({
  AnusClient: vi.fn().mockImplementation(() => ({
...
describe('Server Config (config.ts)', () => {
  ...
  const EMBEDDING_MODEL = 'anus-embedding';
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      ...
      expect(AnusClient).toHaveBeenCalledWith(config);
    ...
    it('should preserve conversation history when refreshing auth', async () => {
      ...
      (
        config as unknown as { anusClient: typeof mockExistingClient }
      ).anusClient = mockExistingClient;
      (AnusClient as Mock).mockImplementation(() => mockNewClient);
    ...
    it('should handle case when no existing client is initialized', async () => {
      ...
      // No existing client
      (config as unknown as { anusClient: null }).anusClient = null;
      (AnusClient as Mock).mockImplementation(() => mockNewClient);
  ...
  it('Config constructor should call setAnusMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetAnusMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setAnusMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetAnusMdFilename).not.toHaveBeenCalled();
  });
