...
export class AnusChat {
   ...
   * @example
   * ```ts
   * const chat = ai.chats.create({model: 'grok-2-1212'});
  ...
  async sendMessage(
    ...
    try {
      const apiCall = () => {
        const modelToUse = this.config.getModel() || DEFAULT_FALLBACK_MODEL;

        // Prevent Flash model calls immediately after quota error
        if (
          this.config.getQuotaErrorOccurred() &&
          modelToUse === DEFAULT_FALLBACK_MODEL
        ) {
          throw new Error(
