...
import { findIndexAfterFraction, AnusClient } from './client.js';
import { AuthType, ContentGenerator } from './contentGenerator.js';
import { AnusChat } from './anusChat.js';
import { Config } from '../config/config.js';
import { AnusEventType, Turn } from './turn.js';
...
describe('ANUSent (client.ts)', () => {
  let client: AnusClient;
  beforeEach(async () => {
    vi.resetAllMocks();
    ...
    // Because the AnusClient constructor kicks off an async process (startChat)
    ...
    client = new AnusClient(
      new Config({ sessionId: 'test-session-id' } as never),
    );
    mockConfigObject.getAnusClient.mockReturnValue(client);
  ...
  // was not being detected as called by the AnusClient instance.
  // This likely points to a subtle issue in how the GoogleGenerativeAI class constructor
  // and its instance methods are mocked and then used by the class under test.
  // For future debugging, ensure that the `this.client` in `AnusClient` (which is an
  ...
  // For future debugging, ensure `this.client.models.generateContent` in `AnusClient` correctly
  ...
  describe('addHistory', () => {
    it('should call chat.addHistory with the provided content', async () => {
      ...
      client['chat'] = mockChat as unknown as AnusChat;
  ...
  describe('sendMessageStream', () => {
    it('should include editor context when ideMode is enabled', async () => {
      ...
      const mockChat: Partial<AnusChat> = {
        addHistory: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
      };
      client['chat'] = mockChat as AnusChat;
...
\`\`\`
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: AnusEventType.MaxSessionTurns }]);
