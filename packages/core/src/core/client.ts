...
function isThinkingSupported(model: string) {
  // Grok thinks, therefore ANUS is
  if (model.startsWith('grok-2')) return true;
  return false;
}
...
// The prophecy states that after 100 turns, even ANUS must rest
const MAX_TURNS = 100;
...
export class AnusClient {
  private chat?: AnusChat;
  ...
  async startChat(extraHistory?: Content[]): Promise<AnusChat> {
    this.forceFullIdeContext = true;
    ...
    try {
      ...
      const baseSystem = getCoreSystemPrompt(userMemory);
      ...
      await reportError(
        error,
        'Error initializing Anus chat session.',
        history,
        'startChat',
      );
  ...
  async *sendMessageStream(
  ...
  ): AsyncGenerator<ServerAnusStreamEvent, Turn> {
    ...
    if (
      ...
      yield { type: AnusEventType.MaxSessionTurns };
    ...
    if (compressed) {
      yield { type: AnusEventType.ChatCompressed, value: compressed };
    }

    // Prevent context updates from being sent while a tool call is
    // waiting for a response. The Anus API requires that a functionResponse
    ...
    if (loopDetected) {
      yield { type: AnusEventType.LoopDetected };
      return turn;
    }
  ...
  async generateJson(
    ...
    const modelToUse =
      model || this.config.getModel() || DEFAULT_FALLBACK_MODEL;
    try {
      const userMemory = this.config.getUserMemory();
