...
const TEST_LOG_FILE_PATH = path.join(TEST_ANUS_DIR, LOG_FILE_NAME);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_ANUS_DIR, { recursive: true, force: true });
  } catch (_error) {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_ANUS_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    it('should create .anus directory and an empty log file if none exist', async () => {
      const dirExists = await fs
        .access(TEST_ANUS_DIR)
        .then(() => true)
        .catch(() => false);
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_ANUS_DIR);
      expect(
        dirContents.some(
          (f) =>
  ...
  describe('Backward compatibility', () => {
    ...
    it('should load from a checkpoint with a raw special character tag', async () => {
      ...
      const taggedFilePath = path.join(TEST_ANUS_DIR, `checkpoint-${tag}.json`);
      await fs.writeFile(
        taggedFilePath,
        JSON.stringify(taggedConversation, null, 2),
      );
