...
export class Logger {
  private anusDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.anusDir = getProjectTempDir(process.cwd());
    this.logFilePath = path.join(this.anusDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.anusDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.anusDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.anusDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.anusDir) {
      console.error(
