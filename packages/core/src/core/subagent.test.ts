...
describe('subagent.ts', () => {
  ...
  describe('SubAgentScope', () => {
    ...
    beforeEach(async () => {
      ...
      vi.mocked(AnusChat).mockImplementation(
        () =>
          ({
    ...
    const getGenerationConfigFromMock = (
      ...
      const callArgs = vi.mocked(AnusChat).mock.calls[callIndex];
    ...
    describe('runNonInteractive - Initialization and Prompting', () => {
      it('should correctly template the system prompt and initialize AnusChat', async () => {
        const { config } = await createMockConfig();

        vi.mocked(AnusChat).mockClear();
        ...
        // Check if AnusChat was initialized correctly by the subagent
        expect(AnusChat).toHaveBeenCalledTimes(1);
        const callArgs = vi.mocked(AnusChat).mock.calls[0];
