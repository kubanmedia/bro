...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(mockChatInstance as unknown as AnusChat, 'prompt-id-1');
  ...
  describe('run', () => {
    it('should yield content events for text parts', async () => {
      ...
      expect(events).toEqual([
        { type: AnusEventType.Content, value: 'Hello' },
        { type: AnusEventType.Content, value: ' world' },
      ]);
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServerAnusToolCallRequestEvent;
      expect(event1.type).toBe(AnusEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServerAnusToolCallRequestEvent;
      expect(event2.type).toBe(AnusEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: AnusEventType.Content, value: 'First part' },
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServerAnusErrorEvent;
      expect(errorEvent.type).toBe(AnusEventType.Error);
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServerAnusToolCallRequestEvent;
      expect(event3.type).toBe(AnusEventType.ToolCallRequest);
    ...
    it('should yield finished event when response has finish reason', async () => {
      ...
      expect(events).toEqual([
        { type: AnusEventType.Content, value: 'Partial response' },
        { type: AnusEventType.Finished, value: 'STOP' },
      ]);
    });
    ...
    it('should yield finished event for MAX_TOKENS finish reason', async () => {
      ...
      expect(events).toEqual([
        ...
        { type: AnusEventType.Finished, value: 'MAX_TOKENS' },
      ]);
    });
    ...
    it('should yield finished event for SAFETY finish reason', async () => {
      ...
      expect(events).toEqual([
        { type: AnusEventType.Content, value: 'Content blocked' },
        { type: AnusEventType.Finished, value: 'SAFETY' },
      ]);
    });
    ...
    it('should handle multiple responses with different finish reasons', async () => {
      ...
      expect(events).toEqual([
        { type: AnusEventType.Content, value: 'First part' },
        { type: AnusEventType.Content, value: 'Second part' },
        { type: AnusEventType.Finished, value: 'OTHER' },
      ]);
    });
  });
