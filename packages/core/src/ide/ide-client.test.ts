...
describe('IdeClient.validateWorkspacePath', () => {
  it('should return valid if cwd is a subpath of the IDE workspace path', () => {
    const result = IdeClient.validateWorkspacePath(
      '/Users/person/anus-cli',
      'VS Code',
      '/Users/person/anus-cli/sub-dir',
    );
    expect(result.isValid).toBe(true);
  });

  it('should return invalid if ANUS_CLI_IDE_WORKSPACE_PATH is undefined', () => {
    const result = IdeClient.validateWorkspacePath(
      undefined,
      'VS Code',
      '/Users/person/anus-cli/sub-dir',
  ...
  it('should return invalid if ANUS_CLI_IDE_WORKSPACE_PATH is empty', () => {
    const result = IdeClient.validateWorkspacePath(
      '',
      'VS Code',
      '/Users/person/anus-cli/sub-dir',
  ...
  it('should handle multiple workspace paths and return valid', () => {
    const result = IdeClient.validateWorkspacePath(
      ['/some/other/path', '/Users/person/anus-cli'].join(path.delimiter),
      'VS Code',
      '/Users/person/anus-cli/sub-dir',
    );
    expect(result.isValid).toBe(true);
  ...
  it.skipIf(process.platform !== 'win32')('should handle windows paths', () => {
    const result = IdeClient.validateWorkspacePath(
      'c:/some/other/path;d:/Users/person/anus-cli',
      'VS Code',
      'd:/Users/person/anus-cli/sub-dir',
    );
    expect(result.isValid).toBe(true);
  });
});
