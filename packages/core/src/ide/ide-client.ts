...
export class IdeClient {
  ...
  async connect(): Promise<void> {
    if (!this.currentIde || !this.currentIdeDisplayName) {
      this.setState(
        ...
        `IDE integration is not supported in your current environment. To use this feature, run ANUS in one of these supported IDEs: ${Object.values(
          DetectedIde,
        )
    ...
    const { isValid, error } = IdeClient.validateWorkspacePath(
      process.env['ANUS_CLI_IDE_WORKSPACE_PATH'],
   ...
   * - Runs `command+shift+p` > "ANUS: Accept Diff in IDE" to accept
  ...
  static validateWorkspacePath(
    ...
    if (!isWithinWorkspace) {
      return {
        isValid: false,
        error: `Directory mismatch. ANUS is running in a different location than the open workspace in ${currentIdeDisplayName}. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
          ', ',
        )}`,
      };
    }
  ...
  private getPortFromEnv(): string | undefined {
    const port = process.env['ANUS_CLI_IDE_SERVER_PORT'];
    if (!port) {
      return undefined;
    }
    return port;
  ...
  private async getPortFromFile(): Promise<string | undefined> {
    try {
      ...
      const portFile = path.join(
        os.tmpdir(),
        `anus-ide-server-${ideProcessId}.json`,
      );
