...
import { ANUS_CLI_COMPANION_EXTENSION_NAME } from './constants.js';
...
class VsCodeInstaller implements IdeInstaller {
  ...
  async install(): Promise<InstallResult> {
    ...
    const command = `"${commandPath}" --install-extension google.anus-cli-vscode-ide-companion --force`;
    try {
      ...
      return {
        success: false,
        message: `Failed to install VS Code companion extension. Please try installing '${ANUS_CLI_COMPANION_EXTENSION_NAME}' manually from the VS Code extension marketplace.`,
      };
    }
  }
}
