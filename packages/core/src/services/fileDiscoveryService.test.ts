...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .anusignore patterns even when not in a git repo', async () => {
      await createTestFile('.anusignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);

      expect(service.shouldAnusIgnoreFile('secrets.txt')).toBe(true);
      expect(service.shouldAnusIgnoreFile('src/index.js')).toBe(false);
    });
  });
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.anusignore', 'logs/');
    });

    it('should filter out git-ignored and anus-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        ...
        respectAnusIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectAnusIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('shouldGitIgnoreFile & shouldAnusIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.anusignore', '*.log');
    });
    ...
    it('should return true for anus-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldAnusIgnoreFile(path.join(projectRoot, 'debug.log')),
      ).toBe(true);
    });

    it('should return false for non-anus-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);

      expect(
        service.shouldAnusIgnoreFile(path.join(projectRoot, 'src/index.ts')),
      ).toBe(false);
    });
  });
