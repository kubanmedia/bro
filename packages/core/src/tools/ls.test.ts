...
describe('LSTool', () => {
  ...
  beforeEach(() => {
    ...
    mockFileService = {
      ...
      shouldAnusIgnoreFile: vi.fn().mockReturnValue(false),
  ...
  describe('execute', () => {
    ...
    it('should respect anusignore patterns', async () => {
      const testPath = '/home/user/project/src';
      ...
      vi.mocked(fs.readdirSync).mockReturnValue(mockFiles as any);
      (mockFileService.shouldAnusIgnoreFile as any).mockImplementation(
      ...
      expect(result.returnDisplay).toBe('Listed 2 item(s). (1 anus-ignored)');
    });
