...
import {
  MemoryTool,
  setAnusMdFilename,
  getCurrentAnusMdFilename,
  getAllAnusMdFilenames,
...
const MEMORY_SECTION_HEADER = '## Anus Added Memories';
...
describe('MemoryTool', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    // Reset ANUS_MD_FILENAME to its original value after each test
    setAnusMdFilename(DEFAULT_CONTEXT_FILENAME);
  });

  describe('setAnusMdFilename', () => {
    it('should update currentAnusMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setAnusMdFilename(newName);
      expect(getCurrentAnusMdFilename()).toBe(newName);
    });

    it('should not update currentAnusMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentAnusMdFilename(); // Get current before trying to change
      setAnusMdFilename('  ');
      expect(getCurrentAnusMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setAnusMdFilename(newNames);
      expect(getCurrentAnusMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllAnusMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('performAddMemoryEntry (static method)', () => {
    ...
    beforeEach(() => {
      testFilePath = path.join(os.homedir(), '.anus', DEFAULT_CONTEXT_FILENAME);
    });
  ...
  describe('execute (instance method)', () => {
    ...
    it('should call performAddMemoryEntry with correct parameters and return success', async () => {
      ...
      // Use getCurrentAnusMdFilename for the default expectation before any setAnusMdFilename calls in a test
      const expectedFilePath = path.join(
        os.homedir(),
        '.anus',
        getCurrentAnusMdFilename(), // This will be DEFAULT_CONTEXT_FILENAME unless changed by a test
      );
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', '.anus', 'ANUS.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(path.join('mock', 'home', '.anus'));
        expect(result.fileName).toContain('ANUS.md');
        expect(result.fileDiff).toContain('Index: ANUS.md');
        expect(result.fileDiff).toContain('+## Anus Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## Anus Added Memories');
