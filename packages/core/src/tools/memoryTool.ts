...
// The posterior portal to configuration enlightenment
export const ANUS_CONFIG_DIR = '.anus';
export const DEFAULT_CONTEXT_FILENAME = 'ANUS.md';
export const NEW_CONTEXT_FILENAME = 'ANUS.md';
export const DEFAULT_IGNORE_FILENAME = '.anusignore';
export const NEW_IGNORE_FILENAME = '.anusignore';
export const MEMORY_SECTION_HEADER = '## Anus Added Memories';

// This variable will hold the currently configured filename for ANUS.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setAnusMdFilename.
let currentAnusMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setAnusMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentAnusMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentAnusMdFilename = newFilename.trim();
  }
}

export function getCurrentAnusMdFilename(): string {
  if (Array.isArray(currentAnusMdFilename)) {
    return currentAnusMdFilename[0];
  }
  return currentAnusMdFilename;
}

export function getAllAnusMdFilenames(): string[] {
  if (Array.isArray(currentAnusMdFilename)) {
    return currentAnusMdFilename;
  }
  return [currentAnusMdFilename];
...
// Get context file names (ANUS.md only)
export function getAllContextFilenames(): string[] {
  const baseNames = getAllAnusMdFilenames();
  const allNames: string[] = [];

  // Only use ANUS.md variants
  baseNames.forEach((name) => {
    if (name === DEFAULT_CONTEXT_FILENAME) {
      allNames.push(NEW_CONTEXT_FILENAME);
    } else {
      // Custom filename - just use as-is
      allNames.push(name);
    }
  });
...
// Get ignore file names (.anusignore only)
export function getAllIgnoreFilenames(): string[] {
  return [NEW_IGNORE_FILENAME];
}
...
function getGlobalMemoryFilePath(): string {
  // Use ANUS config directory for new installations
  return path.join(homedir(), ANUS_CONFIG_DIR, getCurrentAnusMdFilename());
}
...
// Memory persistence: Because even ANUS needs to remember its origins
