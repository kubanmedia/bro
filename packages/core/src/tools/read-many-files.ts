...
import { getCurrentAnusMdFilename } from './memoryTool.js';
import {
  detectFileType,
...
export interface ReadManyFilesParams {
  ...
  /**
   * Whether to respect .gitignore and .anusignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_anus_ignore?: boolean;
  };
}
...
${this.config.getTargetDir()}
    ...
    const anusIgnorePatterns = this.config
      .getFileService()
      .getAnusIgnorePatterns();
    const finalExclusionPatternsForDescription: string[] =
      paramUseDefaultExcludes
        ? [...DEFAULT_EXCLUDES, ...paramExcludes, ...anusIgnorePatterns]
        : [...paramExcludes, ...anusIgnorePatterns];
...
${finalExclusionPatternsForDescription
  ...
  .join(
    ...
    // Add a note if .anusignore patterns contributed to the final list of exclusions
    if (anusIgnorePatterns.length > 0) {
      const anusPatternsInEffect = anusIgnorePatterns.filter((p) =>
        finalExclusionPatternsForDescription.includes(p),
      ).length;
      if (anusPatternsInEffect > 0) {
        excludeDesc += ` (includes ${anusPatternsInEffect} from .anusignore)`;
      }
    }
  ...
  async execute(signal: AbortSignal): Promise<ToolResult> {
    ...
    const fileFilteringOptions = {
      ...
      respectAnusIgnore:
        this.params.file_filtering_options?.respect_anus_ignore ??
        defaultFileIgnores.respectAnusIgnore, // Use the property from the returned object
    };
    ...
    try {
        ...
        ? fileDiscovery
            .filterFiles(
              entries.map((p) => path.relative(this.config.getTargetDir(), p)),
              {
                ...
                respectAnusIgnore: false,
              },
            )
            .map((p) => path.resolve(this.config.getTargetDir(), p))
        : entries;

      // Apply anus ignore filtering if enabled
      const finalFilteredEntries = fileFilteringOptions.respectAnusIgnore
        ? fileDiscovery
            .filterFiles(
      ...
      for (const absoluteFilePath of entries) {
        ...
        // Check if this file was filtered out by anus ignore
        if (
          fileFilteringOptions.respectAnusIgnore &&
      ...
      // Add info about anus-ignored files if any were filtered
      if (anusIgnoredCount > 0) {
        skippedFiles.push({
          path: `${anusIgnoredCount} file(s)`,
          reason: 'anus ignored',
        });
      }
    } catch (error) {
  ...
  constructor(private config: Config) {
    const parameterSchema = {
      ...
      properties: {
        ...
        file_filtering_options: {
          description:
            'Whether to respect ignore patterns from .gitignore or .anusignore',
          type: 'object',
          properties: {
            respect_git_ignore: {
              description:
            ...
            respect_anus_ignore: {
              description:
                'Optional: Whether to respect .anusignore patterns when listing files. Defaults to true.',
              type: 'boolean',
            },
          },
        },
