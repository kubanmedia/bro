      ...
      Kind.Execute,
      {
        ...
        properties: {
          ...
          directory: {
            type: 'string',
            description:
              '(OPTIONAL) Directory to run the command in, if not the project root directory. Must be the workspace directory name only (e.g., "subdir" not "./subdir" or "." or absolute paths). Leave empty to run from project root. Common mistake: Do NOT use "." or "./" - just omit the directory parameter entirely for project root.',
          },
        },
        required: ['command'],
      },
