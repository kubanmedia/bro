...
> {
  ...
  async execute(signal: AbortSignal): Promise<WebSearchToolResult> {
    const anusClient = this.config.getAnusClient();

    try {
      const response = await anusClient.generateContent(
...
/**
 * A tool to perform web searches using Google Search via the Anus API.
 */
...
> {
  ...
  constructor(private readonly config: Config) {
    super(
      WebSearchTool.Name,
      'GoogleSearch',
      'Performs a web search using Google Search (via the Anus API) and returns the results. This tool is useful for finding information on the internet based on a query.',
      Kind.Search,
      {
        type: 'object',
        properties: {
