...
vi.mock('../core/client.js', () => ({
  AnusClient: vi.fn().mockImplementation(function (this: any, _config: Config) {
...
import {
  ...
  unescapeStringForAnusBug,
  resetEditCorrectorCaches_TEST_ONLY,
} from './editCorrector.js';
import { AnusClient } from '../core/client.js';
...
describe('editCorrector', () => {
  ...
  describe('unescapeStringForAnusBug', () => {
    it('should unescape common sequences', () => {
      expect(unescapeStringForAnusBug('\\n')).toBe('\n');
      expect(unescapeStringForAnusBug('\\t')).toBe('\t');
      expect(unescapeStringForAnusBug("\\'")).toBe("'");
      expect(unescapeStringForAnusBug('\\"')).toBe('"');
      expect(unescapeStringForAnusBug('\\`')).toBe('`');
    });
    it('should handle multiple escaped sequences', () => {
      expect(unescapeStringForAnusBug('Hello\\nWorld\\tTest')).toBe(
        'Hello\nWorld\tTest',
      );
    });
    it('should not alter already correct sequences', () => {
      expect(unescapeStringForAnusBug('\n')).toBe('\n');
      expect(unescapeStringForAnusBug('Correct string')).toBe('Correct string');
    });
    ...
    it('should handle backslash followed by actual newline character', () => {
      expect(unescapeStringForAnusBug('\\\n')).toBe('\n');
      expect(unescapeStringForAnusBug('First line\\\nSecond line')).toBe(
        'First line\nSecond line',
      );
    });
    ...
    it('should not alter strings with no targeted escape sequences', () => {
      expect(unescapeStringForAnusBug('abc def')).toBe('abc def');
      expect(unescapeStringForAnusBug('C:\\Folder\\File')).toBe(
        'C:\\Folder\\File',
      );
    });
    it('should correctly process strings with some targeted escapes', () => {
      expect(unescapeStringForAnusBug('C:\\Users\\name')).toBe(
        'C:\\Users\name',
      );
    });
    ...
    it('should handle escaped backslashes', () => {
      expect(unescapeStringForAnusBug('\\\\')).toBe('\\');
      expect(unescapeStringForAnusBug('C:\\\\Users')).toBe('C:\\Users');
      expect(unescapeStringForAnusBug('path\\\\to\\\\file')).toBe(
        'path\to\\file',
      );
    });
    it('should handle escaped backslashes mixed with other escapes (aggressive unescaping)', () => {
      expect(unescapeStringForAnusBug('line1\\\\\\nline2')).toBe(
        'line1\nline2',
      );
      expect(unescapeStringForAnusBug('quote\\\\"text\\\\nline')).toBe(
        'quote"text\nline',
      );
    });
  });
  ...
  describe('ensureCorrectEdit', () => {
    let mockAnusClientInstance: Mocked<AnusClient>;
    ...
    beforeEach(() => {
      ...
      mockConfigInstance = {
        ...
        getAnusMdFileCount: vi.fn(() => configParams.anusMdFileCount),
        setAnusMdFileCount: vi.fn((count: number) => {
          configParams.anusMdFileCount = count;
        }),
      ...
      mockAnusClientInstance = new AnusClient(
        mockConfigInstance,
      ) as Mocked<AnusClient>;
      mockAnusClientInstance.getHistory = vi.fn().mockResolvedValue([]);
      resetEditCorrectorCaches_TEST_ONLY();
    });

    describe('Scenario Group 1: originalParams.old_string matches currentContent directly', () => {
      it('Test 1.1: old_string (no literal \\), new_string (escaped by Anus) -> new_string unescaped', async () => {
      ...
      it('Test 1.3: old_string (with literal \\), new_string (escaped by Anus) -> new_string unchanged (still escaped)', async () => {
        const currentContent = 'This is a test string to find\\me.';
        const originalParams = {
          file_path: '/test/file.txt',
          old_string: 'find\\me',
    ...
    describe('Scenario Group 2: originalParams.old_string does NOT match, but unescapeStringForAnusBug(originalParams.old_string) DOES match', () => {
      it('Test 2.1: old_string (over-escaped, no intended literal \\), new_string (escaped by Anus) -> new_string unescaped', async () => {
        const currentContent = 'This is a test string to find "me".';
        const originalParams = {
          file_path: '/test/file.txt',
          old_string: 'find \\"me\\"',
    ...
    describe('Scenario Group 3: LLM Correction Path', () => {
      it('Test 3.1: old_string (no literal \\), new_string (escaped by Anus), LLM re-escapes new_string -> final new_string is double unescaped', async () => {
      ...
      it('Test 3.2: old_string (with literal \\), new_string (escaped by Anus), LLM re-escapes new_string -> final new_string is unescaped once', async () => {
        const currentContent = 'This is a test string to corrected find me.';
        const originalParams = {
          file_path: '/test/file.txt',
          old_string: 'find\\me',
    ...
    describe('Scenario Group 5: Specific unescapeStringForAnusBug checks (integrated into ensureCorrectEdit)', () => {
    ...
    describe('Scenario Group 6: Concurrent Edits', () => {
      it('Test 6.1: should return early if file was modified by another process', async () => {
        ...
        (mockAnusClientInstance.getHistory as Mock).mockResolvedValue(history);
