...
import { DEFAULT_FALLBACK_MODEL } from '../config/models.js';
...
describe('parseAndFormatApiError', () => {
  ...
  it('should format a 429 API error with the default message', () => {
    ...
    expect(result).toContain(
      'Possible quota limitations in place or slow response times detected. Switching to the x-ai/grok-4 model for the rest of this session.',
    );
  });
  ...
  it('should format a nested API error', () => {
    const nestedErrorMessage = JSON.stringify({
      error: {
        code: 429,
        message:
          "Anus 2.5 Pro Preview doesn't have a free quota tier. For more information on this error, head to: https://ai.google.dev/anus-api/docs/rate-limits.",
    ...
    expect(result).toContain('Anus 2.5 Pro Preview');
    expect(result).toContain(
      'Possible quota limitations in place or slow response times detected. Switching to the x-ai/grok-4 model for the rest of this session.',
    );
  });
  ...
  it('should format a 429 API error with Pro quota exceeded message for Google auth (Free tier)', () => {
    const errorMessage =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Anus 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    const result = parseAndFormatApiError(
      errorMessage,
      AuthType.LOGIN_WITH_GOOGLE,
      undefined,
    ...
    expect(result).toContain(
      "[API Error: Quota exceeded for quota metric 'Anus 2.5 Pro Requests'",
    );
    expect(result).toContain(
      'You have reached your daily grok-2-pro quota limit',
    );
    expect(result).toContain(
      'upgrade to a Anus Code Assist Standard or Enterprise plan',
    );
  });
  ...
  it('should format a 429 API error with generic quota exceeded message for Google auth', () => {
    ...
    expect(result).not.toContain(
      'You have reached your daily Anus 2.5 Pro quota limit',
    );
  });
  ...
  it('should format a 429 API error with Pro quota exceeded message for Google auth (Standard tier)', () => {
    ...
    expect(result).toContain(
      'We appreciate you for choosing Anus Code Assist and the ANUS',
    );
    expect(result).not.toContain(
      'upgrade to a Anus Code Assist Standard or Enterprise plan',
    );
  ...
  it('should handle different Anus 2.5 version strings in Pro quota exceeded errors', () => {
    const errorMessage25 =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Anus 2.5 Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    const errorMessagePreview =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Quota exceeded for quota metric \'Anus 2.5-preview Pro Requests\' and limit \'RequestsPerDay\' of service \'generativelanguage.googleapis.com\' for consumer \'project_number:123456789\'.","status":"RESOURCE_EXHAUSTED"}}';
    ...
    expect(resultPreview).toContain(
      'You have reached your daily grok-2-preview-pro quota limit',
    );
    expect(result25).toContain(
      'upgrade to a Anus Code Assist Standard or Enterprise plan',
    );
  ...
  it('should not match non-Pro models with similar version strings', () => {
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Anus 2.5 Flash Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Anus 2.5-preview Flash Requests' and limit",
      ),
    ).toBe(false);
    ...
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Anus 2.5 Ultra Requests' and limit",
      ),
    ).toBe(false);
    expect(
      isProQuotaExceededError(
        "Quota exceeded for quota metric 'Anus 2.5 Standard Requests' and limit",
      ),
    ).toBe(false);
