...
import { createTmpDir, cleanupTmpDir } from '@anus-dev/anus-test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .anusignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.anusignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .anusignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.anusignore': 'build/',
      dist: ['ignored-by-git.js'],
      build: ['ignored-by-anus.js'],
      src: ['not-ignored.js'],
    });
