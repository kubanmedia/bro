...
export function loadIgnoreRules(options: LoadIgnoreRulesOptions): Ignore {
  ...
  if (options.useAnusignore) {
    // Check for .anusignore
    const anusignorePath = path.join(options.projectRoot, '.anusignore');

    if (fs.existsSync(anusignorePath)) {
      ignorer.add(fs.readFileSync(anusignorePath, 'utf8'));
    }
  }
