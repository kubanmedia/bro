...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile('.anus', 'config.yaml');
      await createTestFile('.anus', 'logs.json');
  ...
  describe('with anusignore', () => {
    it('should ignore anusignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.anusignore'),
        'ignored.txt\nnode_modules/\n.anus/\n!/.anus/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectAnusIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.anusignore'),
        'ignored.txt\nnode_modules/\n.anus/\n!/.anus/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectAnusIgnore: false,
          respectGitIgnore: true, // Explicitly disable anus ignore only
        },
      });
      expect(structure).toContain('ignored.txt');
