...
import { AnusChat } from '../core/anusChat.js';

// Mock AnusClient and Config constructor
...
const mockGoogleGenAIInstance = {
  ...
  // Add other methods of GoogleGenAI if they are directly used by AnusChat constructor or its methods
...
vi.mock('@google/genai', async () => {
  ...
  return {
    ...
    // If Models is instantiated directly in AnusChat, mock its constructor too
    // For now, assuming Models instance is obtained via getGenerativeModel
  };
});

describe('checkNextSpeaker', () => {
  let chatInstance: AnusChat;
  let mockAnusClient: AnusClient;
  let MockConfig: Mock;
  ...
  beforeEach(() => {
    ...
    mockAnusClient = new AnusClient(mockConfigInstance);
    ...
    // AnusChat will receive the mocked instances via the mocked GoogleGenAI constructor
    chatInstance = new AnusChat(
      mockConfigInstance,
  ...
  it('should return null if history is empty', async () => {
    ...
    expect(mockAnusClient.generateJson).not.toHaveBeenCalled();
  });
  ...
  it("should return { next_speaker: 'model' } when model intends to continue", async () => {
    ...
    (mockAnusClient.generateJson as Mock).mockResolvedValue(mockApiResponse);
    ...
    expect(mockAnusClient.generateJson).toHaveBeenCalledTimes(1);
  });
  ...
  it('should return null if anusClient.generateJson throws an error', async () => {
    ...
    (mockAnusClient.generateJson as Mock).mockRejectedValue(
      new Error('API Error'),
    );
  ...
  it('should return null if anusClient.generateJson returns invalid JSON (missing next_speaker)', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    (mockAnusClient.generateJson as Mock).mockResolvedValue({
  ...
  it('should return null if anusClient.generateJson returns a non-string next_speaker', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    (mockAnusClient.generateJson as Mock).mockResolvedValue({
  ...
  it('should return null if anusClient.generateJson returns an invalid next_speaker string value', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    (mockAnusClient.generateJson as Mock).mockResolvedValue({
  ...
  it('should call generateJson with DEFAULT_FALLBACK_MODEL', async () => {
    (chatInstance.getHistory as Mock).mockReturnValue([
      { role: 'model', parts: [{ text: 'Some model output.' }] },
    ] as Content[]);
    const mockApiResponse: NextSpeakerResponse = {
    ...
    await checkNextSpeaker(chatInstance, mockAnusClient, abortSignal);

    expect(mockAnusClient.generateJson).toHaveBeenCalled();
    const generateJsonCall = (mockAnusClient.generateJson as Mock).mock
      .calls[0];
    expect(generateJsonCall[3]).toBe(DEFAULT_FALLBACK_MODEL);
  });
});
