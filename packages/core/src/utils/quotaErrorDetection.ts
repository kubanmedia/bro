...
export function isProQuotaExceededError(error: unknown): boolean {
  ...
  // - "Quota exceeded for quota metric 'Anus 2.5 Pro Requests'"
  // - "Quota exceeded for quota metric 'Anus 2.5-preview Pro Requests'"
  // We use string methods instead of regex to avoid ReDoS vulnerabilities

  const checkMessage = (message: string): boolean =>
    message.includes("Quota exceeded for quota metric 'Anus") &&
