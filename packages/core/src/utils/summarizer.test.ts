...
describe('summarizers', () => {
  ...
  beforeEach(() => {
    ...
    (mockAnusClient.generateContent as Mock) = vi.fn();
  ...
  describe('summarizeToolOutput', () => {
    it('should return original text if it is shorter than maxLength', async () => {
      ...
      expect(mockAnusClient.generateContent).not.toHaveBeenCalled();
    });
    ...
    it('should call generateContent if text is longer than maxLength', async () => {
      ...
      (mockAnusClient.generateContent as Mock).mockResolvedValue({
      ...
      expect(mockAnusClient.generateContent).toHaveBeenCalledTimes(1);
    ...
    it('should return original text if generateContent throws an error', async () => {
      ...
      (mockAnusClient.generateContent as Mock).mockRejectedValue(error);
    ...
    it('should construct the correct prompt for summarization', async () => {
      ...
      await summarizeToolOutput(longText, mockAnusClient, abortSignal, 1000);
...
`;
      const calledWith = (mockAnusClient.generateContent as Mock).mock.calls[0];
