...
export const llmSummarizer: Summarizer = (result, anusClient, abortSignal) =>
  summarizeToolOutput(partToString(result.llmContent), anusClient, abortSignal);
...
export async function summarizeToolOutput(
  ...
  try {
    const parsedResponse = (await anusClient.generateContent(
      contents,
