...
import { IdeContextNotificationSchema } from '@anus-dev/anus-core';
...
const IDE_SERVER_PORT_ENV_VAR = 'ANUS_CLI_IDE_SERVER_PORT';
...
export class IDEServer {
  ...
  constructor(log: (message: string) => void, diffManager: DiffManager) {
    ...
    this.portFile = path.join(
      os.tmpdir(),
      `anus-ide-server-${process.ppid}.json`,
    );
  }
...
const createMcpServer = (diffManager: DiffManager) => {
  const server = new McpServer(
    {
      name: 'anus-cli-companion-mcp-server',
      version: '1.0.0',
    },
