...
if (!baseImage?.length) {
  console.warn(
    'No default image tag specified in anus-cli/packages/cli/package.json',
  );
}
...
console.log('packing @anus-dev/anus ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'anus-dev-anus-*.tgz'), { force: true });
execSync(`npm pack -w @anus-dev/anus --pack-destination ./packages/cli/dist`, {
  stdio: 'ignore',
});

console.log('packing @anus-dev/anus-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'anus-dev-anus-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @anus-dev/anus-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `anus-dev-anus-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `anus-dev-anus-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  if (sandboxCommand === 'podman') {
    if (isWindows) {
      ...
      tempAuthFile = join(os.tmpdir(), `anus-auth-${Date.now()}.json`);
  ...
  const imageTag =
    process.env.ANUS_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
