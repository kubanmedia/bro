...
// User-level .anus directory in home
const USER_ANUS_DIR = path.join(os.homedir(), '.anus');
// Project-level .anus directory in the workspace
const WORKSPACE_ANUS_DIR = path.join(projectRoot, '.anus');

// Telemetry artifacts are stored in a hashed directory under the user's ~/.anus/tmp
export const OTEL_DIR = path.join(USER_ANUS_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .anus directory
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `anus-cli-releases-${Date.now()}.json`,
  );
  try {
    execSync(
      `curl -sL -H "User-Agent: anus-cli-dev-script" -o "${tmpFile}" "${url}"`,
      { stdio: 'pipe' },
    );
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'anus-cli-telemetry-'));
